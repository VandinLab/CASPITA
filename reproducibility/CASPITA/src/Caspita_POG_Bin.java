...
public class Caspita_POG_Bin {
    ...
    public static void main(String[] args) {
        ...
        double FWERTh = Double.parseDouble(args[2]);
        String fileIn = "data/" + dataset + ".csv";
        System.out.println("Dataset: " + dataset);
        System.out.println("P: " + P);
        System.out.println("FWER Threshold: " + FWERTh);
        Path folder = Paths.get("res/POG_Bin/");
        try {
            Files.createDirectories(folder);
        }
        catch (IOException e){
            System.out.println("Error Creating Folders \"res/POG_Bin/\"!");
            System.exit(1);
        }
        for (int k = 2; k <= 5; k++) {
            for (int h = 1; h < k; h++) {
                System.out.println("k: " + k + " h: " + h);
                String fileOutOver = "res/POG_Bin/" + dataset + "_POG_Bin_" + P + "_" + k + "_" + h + "_OV.txt";
                String fileOutUnder = "res/POG_Bin/" + dataset + "_POG_Bin_" + P + "_" + k + "_" + h + "_UN.txt";
                long start = System.currentTimeMillis();
                Caspita_POG_Bin caspita = new Caspita_POG_Bin(fileIn, fileOutOver, fileOutUnder, false);
